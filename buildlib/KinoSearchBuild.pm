...
my $XS_FILEPATH = catfile( 'lib', 'KinoSearch.xs' );
...
sub _gen_pause_exclusion_list {
    my $self = shift;
    my ( @excluded_files, @excluded_dirs );
    opendir( my $dh, '.' ) or die "Can't opendir '.': $!";
    for my $entry ( no_upwards( readdir $dh ) ) {
        next if $entry =~ /^(README|lib)$/;    # allowed
    ...
    # exclude all .pm files without a '=head1 NAME' POD directive
    my $no_visible_pod = $self->_find_files( 'lib', \&_files_without_pod );

    push @excluded_files, @$no_visible_pod;
    chomp(@excluded_files);
    chomp(@excluded_dirs);

    # only exclude files that are actually on-board
    open( my $man_fh, '<', 'MANIFEST' ) or die "Can't open MANIFEST: $!";
    my $manifest = do { local $/; <$man_fh> };
    @excluded_files = grep { $manifest =~ /^$_/m } @excluded_files;
    @excluded_dirs  = grep { $manifest =~ /^$_/m } @excluded_dirs;

    return { file => \@excluded_files, directory => \@excluded_dirs };
}

sub _files_without_pod {
    my $filename = $File::Find::name;
    return unless -f $filename;
    open( my $fh, '<', $filename ) or die "Can't open '$filename': $!";
    my $content = do { local $/; <$fh> };
    return $content !~ /=head1\s*NAME/;
}

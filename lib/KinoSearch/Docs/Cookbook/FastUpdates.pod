...
rewrite of lots of small and/or degraded segments:
    ...
    my $indexer = KinoSearch::Indexer->new( index => '/path/to/index', );
    $indexer->optimize;    # optional
    $indexer->finish;

Calling optimize() is optional; it may improve search-time performance or it
may not, and it will almost always cost more.  But if you're already, say,
taking a machine offline for index consolidation, it may make sense to
optimize() and stretch out the interval before meaningful search-time
...
Like Indexer, BackgroundMerger offers an optimize() option, but it's generally
not a good idea to call it if you're deploying BackgroundMerger often, as it
will cause needless churn.  Instead, simply ask BackgroundMerger to commit(),
and let it decide how much it needs to recycle -- if anything.

...
Conceptually, the files have a hierarchical relationship.
...
a given segment share a common alphanumeric prefix which begins with an
underscore: _0, _a8, _rj.  A fully optimized index has only a single segment.
...
document's stored fields start, then retrieves all of them in one lump.
...
KinoSearch's Term Vector support is in development, and this section is a
placeholder.
...
optimization process that deleted documents truly go away.
...
=head1 Compatibility with Java Lucene

KinoSearch's file format is almost, but not quite, a Lucene ~1.9 compound
index.  If your data is 100% pure ASCII with no null bytes, files generated by
Lucene and KinoSearch will be mutually compatible.  However, if your data does
not follow this strict standard to the letter, indexes written by Lucene will
cause KinoSearch to crash and vice-versa.

The sample code in L<KinoSearch::Docs::Tutorial|KinoSearch::Docs::Tutorial>
produces a Lucene-compatible index.

=head1 SEE ALSO

The technical specification of the Lucene index format is distributed along
with Lucene.  As of this writing, it is also located at
L<http://lucene.apache.org/java/docs/fileformats.html>.

See the section on Strings in the
L<KinoSearch::Docs::DevGuide|KinoSearch::Docs::DevGuide> for a technical
discussion of Lucene compatibility issues and efforts being made to resolve
them.

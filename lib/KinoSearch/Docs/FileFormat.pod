...
A segment's Term List is spread out over several files: one primary file and
...
B<_XXX.tlYYY> - B<T>erm B<L>ist.  Enumerates all the indexed terms for a given
field in the segment.  Terms are sorted by lexical comparison of term text.

=item *

B<_XXX.tlxYYY> - B<T>erm B<L>ist IndeB<X>.  Houses periodic samples from the
main term list -- typically, every 128th term.  Each entry is augmented by a
file pointer into the main term list.

=back


The files follow the naming pattern C<_XXX.tlYYY>, where "_XXX" is the
...
... there would only be 1 pair of term list files per segment, "_XXX.tl0" and
"_XXX.tlx0", containing in this example the terms for the "content" field.


When a Searcher is launched, each .tlx file is decoded and cached as an
...
the same pattern as the term list files, but with a "p": C<_XXX.pYYY>.
...
B<pos_boosts> - Optional. For some applications, it makes sense to store boost
...
C<stored>, its terms will appear in the term list, but its original content
...
the extensions C<tlYYY>, C<tlxYYY>, C<pYYY>, C<ds>, or C<dsx>, unless there is
a live indexing process underway.  What you will find is one file per segment
...
The searcher asks the relevant TermListIndex file, "Do you know anything about

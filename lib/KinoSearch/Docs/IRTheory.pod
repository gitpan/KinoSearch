...
abstract, distilled definitions down with useful traits.  For instance,
a "term" need not be associated with a field name; it doesn't even need to be
made out of text.  However, it is convenient to define it that way when trying
to build a text search engine library.

Similarly, a "posting" in its most rarefied form is simply a term-document
pairing; in KinoSearch, the class L<KinoSearch::Posting::MatchPosting> fills
this role.  However, by associating additional information with a posting like
the number of times the term occurs in the document, we can turn it into a
L<ScorePosting|KinoSearch::Posting::ScorePosting>, making it possible to rank

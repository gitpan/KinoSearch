...
Most people start off by copying the sample applications found in the
C<sample> directory at the root of the KinoSearch distribution and adapting
them to their needs.
...
This tutorial explores how the sample apps work.
...
In addition to C<%fields>, USConSchema supplies an analyzer() subroutine:
...
the one used at index time -- otherwise, KinoSearch will misinterpret the
data and either crash or produce incorrect results.

=head1 Create an index and add content.

Now that we've created a set of rules defining how our index is to be
...
by choosing the Schema factory method C<clobber> (instead of C<open> or
C<create>):

...
they're defined, information which is communicated via subclasses of
L<KinoSearch::Schema::FieldSpec>.  For our search of the US Constitution,
we'll use three fields: C<title>, C<content>, and C<url>.  C<title> and
C<content> will be indexed (the default), so that they can be searched...
...
... but C<url> won't because our urls won't contain any information that we
think will help people find what they're looking for.
...
Try setting the C<url> field so that it gets C<indexed>.
    ...
    package USConSchema::url;
    use base qw( KinoSearch::Schema::FieldSpec );
    # sub indexed {0}  <-- commented out, so uses default true value
...
See L<KinoSearch|KinoSearch> version 0.20_01.

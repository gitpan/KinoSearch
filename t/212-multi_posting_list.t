...
$plist->seek($term);
my ( @temp, @postings );
while ( $plist->bulk_read( \@temp, 1024 ) ) {
    $iters++;
    push @postings, @temp;
}
is( $iters, 3, "one bulk read per segment with valid docs" );
@doc_nums = map { $_->get_doc_num } @postings;
is_deeply( \@doc_nums, [ 2, 27, 77 ], "correct docs from Bulk_Read" );
